...
class Auth
{
     ...
     */
    public function check($name, $uid, $type = 1, $mode = 'model', $relation = 'or')
    {
    ...
    public function getGroups($uid)
    {
        ...
        // 执行查询
        $user_groups = Db::view($auth_group_access, 'aid,group_id')
            ->view($auth_group, 'title,rules', "{$auth_group_access}.group_id={$auth_group}.id", 'LEFT')
            ->where("{$auth_group_access}.aid='{$uid}' and {$auth_group}.status='1'")
            ->select();
    ...
    public function getAuthList($uid, $type)
    {
        ...
        //读取用户组所有权限规则
        $rules = Db::name($this->config['auth_rule'])->where(['type'=>$type,'status' => 1])->where('id','in',$ids)->field('condition,name,brief')->select();
        //循环规则，判断结果。
        $authList = []; //
     ...
     * @return array
     */
    public function getAuthList1($uid, $type)
    {
        static $_authList = []; //保存用户验证通过的权限列表
        $t = implode(',', (array)$type);
        if (isset($_authList[$uid . $t])) {
        ...
        foreach ($rules as $rule) {
            if (!empty($rule['condition'])) {
                ...
                @(eval('$condition=(' . $command . ');'));
                if ($condition) {
                    $authList[] = strtolower($rule['brief']);
                }
            } else {
                //只要存在就记录
                $authList[] = strtolower($rule['brief']);
        ...
        //$data['cids']=explode(',',$groups[0]['cids']);
        $data['rule']=$authList;
        return $data;
    }
...
//        );
        //读取用户组所有权限规则
        //dump($map);
        $rules = Db::name($auth_rule)->where('id','in',$ids)->where(['type' => $type,'status' => 1])->column('id,title,name,condition,brief');
        return $rules;
    }
